...
local noti = require("naughty") -- notifications
...
awful.screen.connect_for_each_screen(function(current_screen)
    ...
    awful.tag({ "main", "vidya", "music", "other" }, current_screen, awful.layout.layouts[1])
    ...
    current_screen.tags_list = awful.widget.taglist {
    ...
    current_screen.tasks_list = awful.widget.tasklist {
        screen  = current_screen,
                ...
                {
                    {
                        ...
                        margins = 1, -- Essentially icon size
    ...
    -- Create the wibar
    current_screen.task_bar = awful.wibar({ position = "top", screen = current_screen, height = 20, border_width = 5 })

    -- Add tasks to the taskbar
    current_screen.task_bar:setup {
        layout = wibox.layout.align.horizontal,
        { -- Left widgets
            layout = wibox.layout.fixed.horizontal,
            mylauncher,
            current_screen.tags_list,
        },
        current_screen.tasks_list, -- Middle widget (current tasks)
        { -- Right widgets
            layout = wibox.layout.fixed.horizontal,
...
globalkeys = gears.table.join(
    awful.key(
        ...
        { description = "show this help menu", group = "misc" }
    ),
    ...
    awful.key(
        {super_key, "Control"}, "r",
        awesome.restart,
        { description = "reload awesome", group = "misc" }
    ),
    awful.key(
        {super_key, "Shift"}, "q",
        awesome.quit,
    ...
    awful.key(
        ...
        { description = "open rofi run prompt", group = "launcher" }
    )
)
...
client.connect_signal(
    ...
    function(current_client)
        ...
        if awesome.startup and not current_client.size_hints.user_position and not current_client.size_hints.program_position then
...
do
    local autostart = {
        "picom",
        "nm-applet",
        "xset r rate 300 50", -- cursor go weeeeee
    }
